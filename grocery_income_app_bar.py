...
LABELS     = list(STORE_LABELS.values())   # ["Whole Foods", "Kroger", ...]
...
# ── Top KPI strip — grouped bar: actual median vs predicted median ────────────
st.subheader("Pipeline results — actual vs predicted median income")

actual_medians = [float(np.median(results[ts]["yt"]))    for ts in STORES]
pred_medians   = [float(np.median(results[ts]["preds"])) for ts in STORES]
maes           = [results[ts]["mae"]                     for ts in STORES]
spears         = [results[ts]["sp"]                      for ts in STORES]
...
fig_kpi.add_trace(go.Bar(
    name="Actual median",
    x=LABELS,
    y=[v/1000 for v in actual_medians],
    marker_color=[rgba(STORE_COLORS[s], 1.0) for s in STORES],
    ...
    hovertemplate=(
        "<b>%{x}</b><br>"
        "Actual median: $%{y:.1f}k<extra></extra>"
    ),
))

# Predicted median bars (hatched effect via lower opacity)
fig_kpi.add_trace(go.Bar(
    name="Predicted median",
    x=LABELS,
    y=[v/1000 for v in pred_medians],
    marker_color=[rgba(STORE_COLORS[s], 0.45) for s in STORES],
    marker_line_color=[STORE_COLORS[s] for s in STORES],
    marker_line_width=2,
    width=0.35,
    offset=0.2,
    hovertemplate=(
        "<b>%{x}</b><br>"
        "Predicted median: $%{y:.1f}k<br>"
        "MAE: $%{customdata[0]:.1f}k<br>"
        "Spearman r: %{customdata[1]:.3f}<extra></extra>"
    ),
    customdata=[[m/1000, s] for m, s in zip(maes, spears)],
))

# Annotate the gap (predicted − actual) above each pair
for i, (ts, act, pred) in enumerate(zip(STORES, actual_medians, pred_medians)):
    gap   = (pred - act) / 1000
    ypos  = max(act, pred) / 1000 + 2
    color = "#27500A" if abs(gap) < 5 else "#791F1F"
...
st.caption(
    "**Solid bars** = actual median income per store  |  "
    "**Faded bars** = model's predicted median  |  "
    "**Annotations** = predicted − actual gap (green < $5k, red ≥ $5k)"
)
